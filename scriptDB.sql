...
CREATE TABLE IF NOT EXISTS `db_poupemais`.`tb_usuario_invest` (
  ...
  CONSTRAINT `fk_tb_usuario_invest_tb_nomePlanos1`
    ...
    REFERENCES `db_poupemais`.`tb_nomePlanos` (`idValorPlano`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tb_usuario_invest_tb_valorPlano1`
    FOREIGN KEY (`idValorPlano`)
    REFERENCES `db_poupemais`.`tb_valorPlano` (`idNomePlano`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
SELECT * FROM tb_planos ORDER BY nomePlano desc;

SELECT idPlano, nomePlano, valorPlano FROM tb_planos;
...
INSERT INTO tb_nomePlano (nomePlano) VALUES ('6 meses');
